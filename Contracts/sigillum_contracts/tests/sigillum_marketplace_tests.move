...
module sigillum_contracts::marketplace_tests {
    ...
    // Test end-to-end flow with a real listing and accept_bid
    #[test]
    public fun test_real_listing_accept_bid() {
        let mut scenario = ts::begin(ADMIN);
        ...
        // Seller accepts the bid
        {
            ...
            assert!(get_total_volume(&marketplace_obj) == 400, 0);
    ...
    // Test buy_now functionality
    #[test]
    public fun test_buy_now() {
        let mut scenario = ts::begin(ADMIN);
        ...
        // Buyer buys it now with the list price
        {
            ts::next_tx(&mut scenario, BUYER1);
            let mut marketplace_obj = ts::take_shared<Marketplace>(&scenario);
    ...
    #[test]
    public fun test_admin_fee_withdrawal() {
        let mut scenario = ts::begin(ADMIN);
        ...
        // Admin withdraws fees
        {
            ...
            let expected_fee = (LIST_PRICE * FEE_PERCENTAGE) / 10000;
            
            marketplace::withdraw_fees(
                &marketplace_cap,
                &mut marketplace_obj,
                expected_fee,
                ADMIN,
